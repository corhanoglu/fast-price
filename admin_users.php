...
<div class="admin-panel">
    ...
    <table>
        <thead><tr><th>Kullanıcı Adı</th><th>Rol</th><th class="actions">İşlemler</th></tr></thead>
        <tbody>
            <?php foreach ($users as $user): ?>
            <tr>
                <td data-label="Kullanıcı Adı"><?php echo htmlspecialchars($user->kullanici_adi); ?></td>
                <td data-label="Rol"><?php echo htmlspecialchars($user->rol); ?></td>
                <td data-label="İşlemler" class="actions">
                    <?php if ($user->rol !== 'admin'): ?>
                        <a href="?action=delete&id=<?php echo $user->id; ?>" class="button button-small button-danger" onclick="return confirm('Bu kullanıcıyı silmek istediğinizden emin misiniz?');" title="Sil"><i class="ph ph-trash"></i></a>
                    <?php else: ?>
                        <span style="color:var(--text-muted); font-size:0.8rem;">Silinemez</span>
                    <?php endif; ?>
                </td>
            </tr>
            <?php endforeach; ?>
